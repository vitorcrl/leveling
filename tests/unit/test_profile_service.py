...
class TestGenerateProfileSummary:
    async def test_valid_json_response_is_parsed(self):
        ...
        result = await generate_profile_summary(make_user(), None, [], client=client)
    ...
    async def test_includes_events_in_prompt(self):
        ...
        await generate_profile_summary(make_user(), None, [event], client=client)

        user_prompt = client.messages.create.call_args.kwargs["messages"][0]["content"]
        assert "essential_expense" in user_prompt
        assert "skipped" in user_prompt

    async def test_includes_goal_in_prompt_when_present(self):
        client = AsyncMock()
        client.messages.create = AsyncMock(return_value=make_claude_response("{}"))
        goal = MagicMock(name="Netflix", goal_value_monthly=Decimal("50"))

        await generate_profile_summary(make_user(), goal, [], client=client)
